...
function CertCard({ cert, onCopy }: { cert: Cert; onCopy: () => void }) {
  const date = cert.date_text || new Date(cert.issued_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
  return (
    <article className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-prime-200 transition-all duration-300 overflow-hidden flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto flex flex-wrap gap-2">
          ...
          <a
            href={`https://www.linkedin.com/profile/add?startTask=CERTIFICATION_NAME&name=${encodeURIComponent(cert.event_name)}&organizationName=${encodeURIComponent(cert.organization)}`}
